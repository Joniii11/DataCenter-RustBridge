param(
    [string[]]$Types = @('RackPosition', 'UsableObject', 'Interact', 'PlayerInteraction', 'PlayerPickUp', 'CarrySystem'),
...
Write-Host "    Rack/Interaction Type Inspector" -ForegroundColor Cyan
Write-Host "  ============================================" -ForegroundColor Cyan
    ...
    foreach ($asmEntry in $assemblies.GetEnumerator()) {
        ...
        foreach ($type in $matched) {
            ...
            if ($interfaces.Count -gt 0) {
                $inames = ($interfaces | ForEach-Object { $_.Name }) -join ', '
                Write-Host "  Implements: $inames" -ForegroundColor Gray
            }
            ...
            foreach ($m in ($methods | Sort-Object { $_.IsStatic }, Name)) {
                ...
                $paramList = @()
                foreach ($par in $m.GetParameters()) {
                    $pn = Format-TypeName $par.ParameterType
                    $paramList += "${pn} $($par.Name)"
                }
