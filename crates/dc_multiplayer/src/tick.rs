...
pub fn update(api: &Api, dt: f32) {
    ...
    if api.version() >= 10 {
        ...
        detect_carry_transitions(api, current_state.num_objects);
    }
...
fn detect_carry_transitions(api: &Api, cur_num: u8) {
    let (connected, loaded, executing) = match with_state(|s| {
        (
            s.connected,
            s.is_host || s.join_state == JoinState::Loaded,
    ...
    if !connected || !loaded || executing {
        with_state(|s| s.prev_carry_count = cur_num);
        return;
    }

    let prev_num = with_state(|s| {
        let p = s.prev_carry_count;
        s.prev_carry_count = cur_num;
        p
    })
    .unwrap_or(0);
    ...
    if prev_num == 0 && cur_num > 0 {
        ...
        with_state(|s| {
            s.held_object_id = id.clone();
            s.held_object_type = obj_type;
        });
    ...
    if prev_num > 0 && cur_num == 0 {
        let suppress = with_state(|s| {
            if s.suppress_next_drop {
                s.suppress_next_drop = false;
                true
            } else {
                false
            }
        ...
        if suppress {
            ...
            with_state(|s| {
                s.held_object_id.clear();
                s.held_object_type = 0;
            });
            return;
        }

        let (id, obj_type) = with_state(|s| {
            let r = (s.held_object_id.clone(), s.held_object_type);
            s.held_object_id.clear();
            s.held_object_type = 0;
            r
        })
